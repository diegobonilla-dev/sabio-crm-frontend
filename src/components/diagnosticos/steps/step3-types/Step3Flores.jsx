...
/**
 * Paso 3: Fertilización y Fumigación - Flores
 *
 * Estructura:
 * - Sección A: Información General (toda la finca)
 * - Sección B: Manejo Diferencial Opcional (por bloques)
 */
export default function Step3Flores({ data, onChange }) {
  ...
  const {
    ...
    defaultValues: data?.datos_flores?.fertilizacion_fumigacion || {
      general: {
  ...
  useEffect(() => {
    if (data?.datos_flores?.fertilizacion_fumigacion) {
      reset(data.datos_flores.fertilizacion_fumigacion);
    }
  ...
  const handleAddLoteDiferenciado = () => {
    const newLote = {
      nombre_lote: `Bloque ${lotesDiferenciados.length + 1}`,
      usa_fertilizacion_quimica: false,
      productos_quimicos: [],
      usa_abono_organico: false,
      usa_fumigacion: false,
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h2 className="text-xl md:text-2xl font-bold">Fertilización y Fumigación - Flores</h2>
        <p className="text-sm md:text-base text-gray-600">Información general y manejo diferencial por bloque</p>
      </div>
      ...
      <div className="border-2 border-purple-200 rounded-lg p-4 md:p-6 bg-purple-50/30">
        <h3 className="text-lg md:text-xl font-bold mb-4 text-purple-900">Sección B: Manejo Diferencial por Bloque (Opcional)</h3>

        {/* Pregunta inicial */}
        <div className="bg-white border border-purple-200 rounded-lg p-4 mb-4">
          <Label htmlFor="tiene_manejo_diferencial" className="text-base font-semibold">
            ¿Tiene manejo diferencial de fertilización/fumigación por bloques?
          </Label>
          <Select
          ...
          <p className="text-xs text-gray-500 mt-2">
            Si maneja algunos bloques de forma diferente al resto, seleccione &quot;Sí&quot;
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <Label htmlFor="cuantos_lotes_diferenciados" className="text-base font-semibold">
                ¿Cuántos bloques tienen manejo diferencial?
              </Label>
              <Input
            ...
            {lotesDiferenciados.length > 0 && (
              <div className="space-y-3">
                <h4 className="text-base md:text-lg font-semibold">Bloques con manejo diferencial ({lotesDiferenciados.length})</h4>
                        ...
                        <div className="flex items-center justify-between p-3 md:p-4 cursor-pointer hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                if (confirm(`¿Eliminar "${watch(`lotes_diferenciados.${index}.nombre_lote`) || `Bloque ${index + 1}`}"?`)) {
                                  removeLote(index);
            ...
            {lotesDiferenciados.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                <p className="text-sm md:text-base">No hay bloques diferenciados. Haga clic en &quot;Agregar bloque&quot; para comenzar.</p>
              </div>
            )}
          </div>
        )}
...
function LoteDiferenciadoForm({ index, register, control, setValue, watch, errors }) {
  ...
  return (
    <div className="space-y-4">
      {/* Nombre del bloque */}
      <div>
        <Label htmlFor={`nombre_lote_${index}`}>Nombre/Número del bloque *</Label>
        <Input
      ...
      {/* ¿Usa fertilización química? */}
      <div>
        <Label>¿Usa fertilización química en este bloque?</Label>
        <Select
      ...
      {usaFertilizacionQuimica && (
        <div className="space-y-3 pl-4 border-l-4 border-green-500">
          <div className="flex items-center justify-between">
            <Label className="text-sm font-semibold">Productos químicos en este bloque</Label>
            <Button
              type="button"
              variant="outline"
              size="sm"
      ...
      {/* ¿Usa fumigación? */}
      <div>
        <Label>¿Usa fumigación en este bloque?</Label>
        <Select
      ...
      {usaFumigacion && (
        <div className="space-y-3 pl-4 border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            <Label className="text-sm font-semibold">Insecticidas en este bloque</Label>
            <Button
              type="button"
              variant="outline"
              size="sm"
      ...
      {/* Observaciones */}
      <div>
        <Label>Observaciones de este bloque</Label>
        <Input
