...
export default function Step4Default({ data, onChange }) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      onChange({ manejo_cultivo: formValues });
    }, 300);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex-1">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800 mb-2">
                <strong>⚠️ En desarrollo:</strong> Este formulario se adaptará según el tipo de cultivo.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-purple-50/30 border border-purple-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-purple-900 mb-2">📊 Evaluación por Divisiones</h3>
        <p className="text-sm text-purple-800">Próximamente: evaluación por divisiones con campos específicos.</p>
      </div>
    </div>
  );
}
